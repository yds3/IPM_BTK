...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.goal_embedding = nn.Embedding(config.word_max_goal,config.hidden_size)

        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)


    def forward(self, input_ids, token_type_ids=None, position_ids=None, goal_ids=None):
        seq_length = input_ids.size(1)
        goal_seq_length = goal_ids.size(1)
        ...
        goal_position_ids = torch.arange(goal_seq_length, dtype=torch.long, device=goal_ids.device)
        goal_position_ids = goal_position_ids.unsqueeze(0).expand_as(goal_ids)

        goal_token_type_ids = torch.zeros_like(goal_ids)
        ...
        goal_words_embeddings = self.word_embeddings(goal_ids)
        goal_position_embeddings = self.position_embeddings(goal_position_ids)
        goal_token_type_embeddings = self.token_type_embeddings(goal_token_type_ids)
        goal_embeddings = goal_words_embeddings + goal_position_embeddings + goal_token_type_embeddings
        goal_embeddings = self.LayerNorm(goal_embeddings)
        goal_embeddings = self.dropout(goal_embeddings)
...
class LanguageEncoder(nn.Module):
    ...
    def forward(self, txt_embeds, txt_masks, goal_embeds=None, goal_masks=None):
        extended_txt_masks = extend_neg_masks(txt_masks)
        # print(f"xiaoshu txt_embeds {txt_embeds}")
        for layer_module in self.layer:
            temp_output = layer_module(txt_embeds, extended_txt_masks)
            txt_embeds = temp_output[0]
            # print(f"txt_embeds1111111111111111 {txt_embeds}")
        if not self.update_lang_bert:
            txt_embeds = txt_embeds.detach()

        if goal_embeds is not None: 
            # print(f"goal_embeds {goal_embeds}")
            goal_masks = extend_neg_masks(goal_masks) # Only use object tokens for REVERIE since the action tokens are not obvious in REVERIE.
            instr_aug_embeds = self.instr_actobj_encoder(txt_embeds, goal_embeds, goal_masks)[0]
...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.knowledges_vp_encoder = CrossmodalEncoder(knowledge_config)
    ...
    def forward_text(self, txt_ids, txt_masks, goal_ids=None, goal_masks=None):
        txt_token_type_ids = torch.zeros_like(txt_ids)
        # print(f"     goal_ids: {goal_ids}\n")
        txt_embeds, goal_embeds = self.embeddings(txt_ids, token_type_ids=txt_token_type_ids,
         goal_ids=goal_ids)
        txt_embeds = self.lang_encoder(txt_embeds, txt_masks, goal_embeds, goal_masks)
        return txt_embeds
    ...
    def forward_panorama_per_step(
        ...
        # print(f"view_img_fts {view_img_fts.shape}")
    ...
    #     vp_img_embeds, vp_pos_fts, vp_masks, vp_nav_masks, vp_obj_masks, vp_cand_vpids,knowledge_fts,
    #     ima_know_fts):
    #     batch_size = txt_embeds.size(0)
    #     print(f"txt_embeds {txt_embeds.shape}")
    #     print(f"gmap_img_embeds {gmap_img_embeds.shape}")
    #     print(f"vp_img_embeds {vp_img_embeds.shape}")
    #     print(f"knowledge_fts {knowledge_fts.shape}")
    #     print(f"ima_know_fts {ima_know_fts.shape}")

    #     ima_knowledges_embeds = self.knowledge_proj(ima_know_fts.view(batch_size, -1, 512)) 
    #     print(f"ima_knowledges_embeds {ima_knowledges_embeds.shape}")

    #     # print(f"vp_embeds{vp_embeds.shape}")
      
    #     txt_know_aug_embeds = self.knowledges_vp_encoder(ima_knowledges_embeds, None, txt_embeds, txt_masks)

    #     aug_weight = self.sigmoid(self.knowledges_linear(txt_know_aug_embeds) + self.txt_linear(txt_embeds))
    #     txt_embeds = torch.mul(aug_weight, txt_embeds) + torch.mul((1 - aug_weight), txt_know_aug_embeds)
    #     print(f"txt_embeds2 {txt_embeds.shape}")
    ...
    #     knowledges_embeds = self.knowledge_proj(knowledge_fts.view(batch_size, -1, 512)) 

    #     print(f"knowledges_embeds {knowledges_embeds.shape}")




    #     vp_aug_embeds = self.knowledges_vp_encoder(knowledges_embeds, None, vp_embeds, vp_masks)

    #     aug_weight = self.sigmoid(self.knowledges_linear(vp_aug_embeds) + self.txt_linear(vp_embeds))
    #     vp_embeds = torch.mul(aug_weight, vp_embeds) + torch.mul((1 - aug_weight), vp_aug_embeds)
    ...
    def forward_navigation_per_step(
        ...
        # ima_knowledges_embeds = self.knowledge_proj(ima_know_fts)
        ima_knowledges_embeds = self.knowledge_proj(ima_know_fts.view(batch_size, -1, 512))
        # print(f"knowledges_embeds {knowledges_embeds.shape}")
        # print(f"ima_knowledges_embeds {ima_knowledges_embeds.shape}")
        # print(f"txt_embeds {txt_embeds.shape}")
...
# jinghua
        ima_knowledge_matrix = torch.matmul(ima_knowledges_embeds, txt_embeds.transpose(1, 2))  # [16, 36, 45]

        ima_knowledge_weight = torch.softmax(ima_knowledge_matrix / math.sqrt(768),dim=-1)
        ima_knowledge_fts = torch.matmul(ima_knowledge_weight, txt_embeds)

        ima_knowledges_embeds = self.knowledge_proj(ima_knowledge_fts.view(batch_size, -1, 512))
# sigmoid
        txt_know_aug_embeds = self.knowledges_vp_encoder(ima_knowledges_embeds, None, txt_embeds, txt_masks)
    ...
    def forward(self, mode, batch, **kwargs):
        if mode == 'language':
            txt_embeds = self.forward_text(batch['txt_ids'], batch['txt_masks'],
                batch['goal_ids'], batch['goal_masks'])
            return txt_embeds
            ...
            return pano_embeds, pano_masks
             ...
             return self.forward_navigation_per_step(
                ...
                batch['vp_nav_masks'], batch['vp_obj_masks'], batch['vp_cand_vpids'], batch['knowledge_fts'], batch['ima_know_fts']
            )

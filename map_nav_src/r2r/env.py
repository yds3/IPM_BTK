...
class EnvBatch(object):
    ...
    def __init__(self, connectivity_dir, scan_data_dir=None, feat_db=None, batch_size=100):
        ...
        self.img_knowledge_db = FeaturesDB("../VLN/VLN-DUET-main/datasets/Knowledge/R2R_pan.hdf5")
        self.knowledge_db = FeaturesDB("../VLN/VLN-DUET-main/datasets/Knowledge/captions_output.hdf5")
        self.viewpoint_knowledge = json.load(open('../VLN/VLN-DUET-main/datasets/Knowledge/knowledge.json','r'))
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            for vp_index in range(36):
                ...
                for j in range(CROP_SIZE):
                    knowledge_ids = self.env.viewpoint_knowledge[key+'_'+str(vp_index)+'_'+str(j)][:5]
                    for k in knowledge_ids:
                        crop_feature = self.env.knowledge_db.get_feature(str(k))
                        view_feature.append(crop_feature.reshape(1,512))
                view_feature = np.concatenate(view_feature,axis=0)
                
                knowledge_feature.append(view_feature.reshape(1,5*CROP_SIZE,512))

            knowledge_feature = np.concatenate(knowledge_feature,axis=0)

            img_know_key = item['img_know_dir']
            ima_know_feature = self.env.img_knowledge_db.get_feature(img_know_key).reshape(36,CROP_SIZE,512)
            # print(f"ima_know_feature{ima_know_feature}")

            # print(f"img_know_key {img_know_key}")
            # print(f"item['goal'] {item['goal']}")
            ...
            ob = {
                ...
                'ima_know_feature': ima_know_feature

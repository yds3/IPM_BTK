...
class GMapNavAgent(Seq2SeqAgent):
    ...
    def _language_variable(self, obs):
        ...
        goal_seq_lengths = [len(ob['goal']) for ob in obs]
        goal_seq_tensor = np.zeros((len(obs), max(goal_seq_lengths)), dtype=np.int64)
        goal_mask = np.zeros((len(obs),max(goal_seq_lengths)),dtype=np.bool)


        for i, ob in enumerate(obs):
            goal_seq_tensor[i, :goal_seq_lengths[i]] = ob['goal']
            goal_mask[i, :goal_seq_lengths[i]] = True
                  
        goal_seq_tensor = torch.from_numpy(goal_seq_tensor).long().cuda()
        goal_mask = torch.from_numpy(goal_mask).cuda()

        seq_tensor = torch.from_numpy(seq_tensor).long().cuda()
        mask = torch.from_numpy(mask).cuda()
        return {
            'txt_ids': seq_tensor, 'txt_masks': mask,
            'goal_ids': goal_seq_tensor, 'goal_masks': goal_mask,
        }
    ...
    def _panorama_feature_variable(self, obs):
        ...
        for i, ob in enumerate(obs):
            ...
            # knowledge
            knowledge_fts.append(ob['knowledge_feature'].reshape(1,36,5*CROP_SIZE,512))

            ima_know_fts.append(ob['ima_know_feature'].reshape(1,36,CROP_SIZE,512))

        knowledge_fts = torch.tensor(np.concatenate(knowledge_fts,axis=0)).cuda()
        ima_know_fts = torch.tensor(np.concatenate(ima_know_fts,axis=0)).cuda()

        # print(f"knowledge_fts{knowledge_fts}")
        ...
        return {
            ...
            'cand_vpids': batch_cand_vpids,'knowledge_fts': knowledge_fts,
            'ima_know_fts':ima_know_fts        }
    ...
    def rollout(self, train_ml=None, train_rl=False, reset=True):
        ...
        for t in range(self.args.max_action_len):
            ...
            nav_inputs.update({
                ...
                'knowledge_fts': pano_inputs['knowledge_fts'],
                'ima_know_fts':pano_inputs['ima_know_fts']
            })
